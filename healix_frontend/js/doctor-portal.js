...
function appendDoctorMessage(msg) {
  const box = document.getElementById('docChatMessages');
  if (!box) return;
...
function sendChatMessage(username) {
  ...
  window._docChatOptimisticMsgs = window._docChatOptimisticMsgs || [];
  window._docChatOptimisticMsgs.push(txt);
