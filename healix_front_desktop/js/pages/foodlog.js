pages['foodlog'] = {
  ...
  search: async () => {
    ...
    box.innerHTML = res.data.data.map(f => `
      <div style="background:var(--card2);padding:8px 12px;border-radius:6px;border:1px solid var(--border);cursor:pointer;display:flex;justify-content:space-between"
           onclick="pages.foodlog.fill(${f.food_id}, '${f.food_name}')">
        <span style="font-size:13px;font-weight:600">${f.food_name}</span>
        <span style="font-size:11px;color:var(--sub)">${f.serving_size}</span>
      </div>
    `).join('');
  },
  fill: async (id, name) => {
